...
{
	/// <summary>
	/// Defines extended business logic and manages access to the INSTRUMENT_PART_HISTORY entity.
	/// </summary>
	[SampleManagerEntity(EntityName)]
	public class InstrumentPartHistory : BaseEntity
	{
		...
		/// </summary>
		public const string EntityName = "INSTRUMENT_PART_HISTORY";
		...
		/// <summary>
		/// Gets a value indicating whether the instrument part is available
		/// </summary>
		...
		/// <summary>
		/// Gets a value indicating whether the instrument part is retired
		/// </summary>
		...
		/// <summary>
		/// Gets the status of the instrument part
		/// </summary>
		/// <value>The instruments status.</value>
		[PromptValidPhrase(PhraseInstpStat.Identity)]
		...
		/// <summary>
		/// Gets a value indicating whether the instrument part requires calibration
		/// </summary>
		...
		/// <summary>
		/// Gets a value indicating whether the instrument part requires servicing
		/// </summary>
		...
		static public IEntityCollection BuildHistory(IEntityManager entityManager, string tableName, string identity)
		{
			...
			InstrumentPartHistory ipHistory = null;
			...
			foreach (AuditValues auditValue in auditValuesCollection)
			{
				if ((auditValue.Event.Event != currentEvent) || (ipHistory == null))
				{
					...
					ipHistory = (InstrumentPartHistory)entityManager.CreateEntity(EntityName);
				...
				switch (auditValue.FieldName.Trim())
				{
					case "AVAILABLE":
						if (!BlankValue(auditValue.ValueAfter))
							ipHistory.m_Available = (auditValue.ValueAfter.ToUpper().CompareTo("TRUE") == 0);
						else if (!BlankValue(auditValue.ValueBefore))
							ipHistory.m_Available = (auditValue.ValueBefore.ToUpper().CompareTo("TRUE") == 0);
						break;

					case "RETIRED":
						if (!BlankValue(auditValue.ValueAfter))
							ipHistory.m_Retired = (auditValue.ValueAfter.ToUpper().CompareTo("TRUE") == 0);
						else if (!BlankValue(auditValue.ValueBefore))
							ipHistory.m_Retired = (auditValue.ValueBefore.ToUpper().CompareTo("TRUE") == 0);
						break;

					case "STATUS":
						if (!BlankValue(auditValue.ValueAfter))
							ipHistory.m_Status = (PhraseBase)entityManager.SelectPhrase(PhraseInstpStat.Identity, auditValue.ValueAfter);
						else if (!BlankValue(auditValue.ValueBefore))
							ipHistory.m_Status = (PhraseBase)entityManager.SelectPhrase(PhraseInstpStat.Identity, auditValue.ValueBefore);
						break;

					case "REQUIRES_CALIBRATION":
						if (!BlankValue(auditValue.ValueAfter))
							ipHistory.m_RequiresCalibration = (auditValue.ValueAfter.ToUpper().CompareTo("TRUE") == 0);
						else if (!BlankValue(auditValue.ValueBefore))
							ipHistory.m_RequiresCalibration = (auditValue.ValueBefore.ToUpper().CompareTo("TRUE") == 0);
						break;

					case "LAST_CALIB_DATE":
						if (!BlankValue(auditValue.ValueAfter))
							ipHistory.m_CalibrationDate = auditValue.ValueAfter;
						else if (!BlankValue(auditValue.ValueBefore))
							ipHistory.m_CalibrationDate = auditValue.ValueBefore;
						break;

					case "REQUIRES_SERVICING":
						if (!BlankValue(auditValue.ValueAfter))
							ipHistory.m_RequiresService = (auditValue.ValueAfter.ToUpper().CompareTo("TRUE") == 0);
						else if (!BlankValue(auditValue.ValueBefore))
							ipHistory.m_RequiresService = (auditValue.ValueBefore.ToUpper().CompareTo("TRUE") == 0);
						break;

					case "LAST_SERVICE_DATE":
						if (!BlankValue(auditValue.ValueAfter))
							ipHistory.m_ServiceDate = auditValue.ValueAfter;
						else if (!BlankValue(auditValue.ValueBefore))
							ipHistory.m_ServiceDate = auditValue.ValueBefore;
						break;
